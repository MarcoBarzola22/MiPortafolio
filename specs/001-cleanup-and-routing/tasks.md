...
- [x] **T1 — Registrar baseline del bundle.**
  RF: RF-01 (RNF-02).
...
- [x] **T2 — Eliminar directorio `src/components/ui/` completo.**
  RF: RF-01.
...
- [x] **T3 — Eliminar archivos huérfanos: `NavLink.tsx`, `NotFound.tsx`, `use-toast.ts`, `App.css`.**
  RF: RF-01, RF-02.
...
- [x] **T4 — Eliminar `src/lib/utils.ts` y verificar que ningún componente de negocio lo importa.**
  RF: RF-01.
  Borrar `src/lib/utils.ts` (función `cn` con deps `clsx` + `tailwind-merge`).
  Hecho cuando: `grep -r "lib/utils" src/` no devuelve resultados.

- [x] **T5 — Evaluar y decidir sobre `src/hooks/use-mobile.tsx`.**
  RF: RF-01.
...
- [x] **T6 — Eliminar archivos de configuración huérfanos en la raíz.**
  RF: RF-01.
  Eliminar `components.json` y `bun.lockb`.
  Hecho cuando: `ls components.json bun.lockb` confirma que no existen.

- [x] **T7 — Limpiar `vite.config.ts`: eliminar `lovable-tagger`.**
  RF: RF-01.
...
- [x] **T8 — Limpiar `tailwind.config.ts`: eliminar tokens y plugins huérfanos.**
  RF: RF-01.
...
- [x] **T9 — Limpiar `index.css`: eliminar variables CSS huérfanas.**
  RF: RF-01.
...
- [x] **T10 — Desinstalar todas las dependencias a purgar vía npm.**
  RF: RF-01.
...
- [x] **T11 — Ejecutar build de verificación post-purga.**
  RF: RF-01.
...
- [x] **T12 — Reescribir `App.tsx`: eliminar wrappers de Lovable.**
  RF: RF-02.
...
- [x] **T13 — Consolidar orquestador: mover lógica de `Index.tsx` y eliminar directorio `pages/`.**
  RF: RF-02.
...
- [x] **T14 — Build de verificación post-recableado.**
  RF: RF-02.
...
- [x] **T15 — Refactorizar estado del orquestador: reemplazar `useState` por `useReducer`.**
  RF: RF-03, RF-07.
...
- [x] **T16 — Separar la barra `<nav>` del masthead y hacerla sticky.**
  RF: RF-04.
...
- [x] **T17 — Implementar hook `useKeyboardNavigation`.**
  RF: RF-05.
...
- [x] **T18 — Implementar hook `useSwipeDetection`.**
  RF: RF-06, RF-09.
...
- [x] **T19 — Implementar animación de rebote en límites.**
  RF: RF-08.
...
- [x] **T20 — Build de verificación post-navegación.**
...
- [x] **T21 — Eliminar keyframes duplicados de `tailwind.config.ts` y consolidar en `index.css`.**
  RF: RF-07.
...
- [x] **T22 — Refactorizar orquestación de animación: `onAnimationEnd` reemplaza `setTimeout`.**
  RF: RF-07.
...
- [x] **T23 — Implementar soporte para `prefers-reduced-motion`.**
  RF: RF-07 (RNF-03).
...
- [x] **T24 — Evaluar animación `unfold` y su uso de `filter: blur()`.**
  RF: RF-07 (RNF-01).
...
- [x] **T25 — Build de verificación post-animación.**
  RF: RF-07.
...
- [x] **T26 — Actualizar React y React-DOM a v19.**
  RF: RF-10.
...
- [x] **T27 — Resolver conflictos de peer dependencies.**
  RF: RF-10.
...
- [x] **T28 — Revisar y adaptar breaking changes de React 19.**
  RF: RF-10.
...
- [x] **T29 — Build final y verificación completa.**
  RF: RF-10 (todos).
...
- [x] **T30 — Registrar tamaño final del bundle y comparar con baseline.**
  RF: RNF-02.
...
|---|---|---|---|
| Baseline (T1) | 104.73 kB (329.72 kB raw) | 12.28 kB (67.72 kB raw) | 117.01 kB gzip (397.44 kB raw) |
| Post-purga (T11) | 62.29 kB (202.44 kB raw) | 4.88 kB (19.40 kB raw) | 67.17 kB gzip (221.84 kB raw) |
| Final (T30) | 86.52 kB (285.67 kB raw) | 5.33 kB (21.60 kB raw) | 91.85 kB gzip (307.27 kB raw) |
| **Reducción** | **-17.4%** (-13.4% raw) | **-56.6%** (-68.1% raw) | **-21.5% gzip** (-22.7% raw) |
